...
import { getDatabase, ref, push, onValue, remove, serverTimestamp, query, limitToLast, orderByChild }
  from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js";

const firebaseConfig = {
  apiKey:            "AIzaSyCv4xGhEgjIbWkhq6E4qtaOlfHfNH6jkrQ",
  authDomain:        "asra-2226.firebaseapp.com",
  databaseURL:       "https://asra-2226-default-rtdb.firebaseio.com",
  projectId:         "asra-2226",
...
// ============================================================
// STUDENT DATA — CSE Branch Code 5
// ============================================================
const students = [
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { closeModal(); closeMediaLightbox(); }
});
...
onValue(galleryRef, snapshot => {
  ...
  Object.entries(data).forEach(([key, item]) => {
    ...
    if (isYT) {
      ...
      tile.innerHTML = `
        <video src="${item.src}" muted loop playsinline preload="metadata" style="width:100%;aspect-ratio:4/3;object-fit:cover;cursor:pointer"></video>
        <div class="gallery-tile-bar">
          <span class="gallery-tile-caption">${item.caption || ''}</span>
          <button class="gallery-tile-del" title="Delete">🗑</button>
        </div>`;
...
// ============================================================
const chatRef   = query(ref(db, 'chat'), orderByChild('ts'), limitToLast(100));
const chatBox   = document.getElementById('chatBox');
const chatInput = document.getElementById('chatMsgInput');
const chatStatus = document.getElementById('chatStatus');

// Name persistence
let chatName = localStorage.getItem('asra_chat_name') || '';
const nameBar     = document.getElementById('chatNameBar');
const nameDisplay = document.getElementById('chatNameDisplay');
const nameShown   = document.getElementById('chatNameShown');

function applyChatName() {
  if (chatName) {
    nameBar.style.display     = 'none';
    nameDisplay.style.display = 'flex';
    nameShown.textContent     = `Chatting as: ${chatName}`;
  } else {
...
document.getElementById('chatNameSave').addEventListener('click', () => {
  ...
  if (!val) return;
  chatName = val;
  localStorage.setItem('asra_chat_name', chatName);
  applyChatName();
});
document.getElementById('chatNameChange').addEventListener('click', () => {
  chatName = '';
  localStorage.removeItem('asra_chat_name');
...
function sendMessage() {
  ...
  if (!text) return;
  if (!chatName) { chatStatus.textContent = '⚠️ Set your name first!'; setTimeout(() => chatStatus.textContent = '', 2500); return; }
  push(ref(db, 'chat'), { name: chatName, text, ts: Date.now() })
    .then(() => { chatInput.value = ''; })
    .catch(() => { chatStatus.textContent = '❌ Failed to send — check Firebase config.'; });
}
...
onValue(chatRef, snapshot => {
  ...
  const data = snapshot.val();
  if (!data) {
    chatBox.innerHTML = '<div class="chat-empty">No messages yet — say hi! 👋</div>';
    return;
  }
  Object.values(data).forEach(msg => {
    const bubble = document.createElement('div');
    const isMe   = msg.name === chatName;
    bubble.className = `chat-bubble ${isMe ? 'chat-me' : 'chat-other'}`;
    const time = new Date(msg.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    bubble.innerHTML = `
      <div class="chat-sender">${isMe ? 'You' : msg.name}</div>
